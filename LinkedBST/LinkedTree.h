...
public:
	...
	TreeNode<T> *findSuccessor(TreeNode<T> *t_root);
...
template <class T>
TreeNode<T> *LinkedTree<T>::findSuccessor(TreeNode<T> *t_root) // RST
{
	...
	if (ptr != 0)
	{
		...
		TreeNode<T> * parent = findParent(t_root->getInfo());
		while(t_root != 0 && t_root != parent->getLeft()){
			t_root = parent;
			TreeNode<T> * parent = findParent(t_root->getInfo());

		}
		if(t_root == parent->getLeft()){
			return parent;
		}
	}
}
